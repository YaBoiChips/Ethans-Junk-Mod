...
import yaboichips.JunkMod.entities.HolyHandEntity;

import javax.annotation.Nullable;

public class HolyHandGrenade extends Block {
    public static final BooleanProperty UNSTABLE = BlockStateProperties.UNSTABLE;

    public HolyHandGrenade(Block.Properties properties) {
        super(properties);
    ...
    public static void explode(World p_196534_0_, BlockPos worldIn) {
        explode(p_196534_0_, worldIn, null);
    }

    @Deprecated //Forge: Prefer using IForgeBlock#catchFire
    private static void explode(World worldIn, BlockPos pos, @Nullable LivingEntity entityIn) {
        if (!worldIn.isRemote) {
            HolyHandEntity holyhandentity = new HolyHandEntity(worldIn, (double) pos.getX() + 0.5D, pos.getY(), (double) pos.getZ() + 0.5D, entityIn);
            worldIn.addEntity(holyhandentity);
            worldIn.playSound(null, holyhandentity.getPosX(), holyhandentity.getPosY(), holyhandentity.getPosZ(), SoundEvents.ENTITY_TNT_PRIMED, SoundCategory.BLOCKS, 1.0F, 1.0F);
        }
    }
    ...
    public void onExplosionDestroy(World worldIn, BlockPos pos, Explosion explosionIn) {
        if (!worldIn.isRemote) {
            HolyHandEntity holyhandentity = new HolyHandEntity(worldIn, (float) pos.getX() + 0.5F, pos.getY(), (float) pos.getZ() + 0.5F, explosionIn.getExplosivePlacedBy());
            holyhandentity.setFuse((short) (worldIn.rand.nextInt(holyhandentity.getFuse() / 4) + holyhandentity.getFuse() / 8));
            worldIn.addEntity(holyhandentity);
        }
    }
